...
const sketch = (s: p5SVG) => {
    ...
    s.draw = () =>  {
        ...
        s.stroke(255);
        s.noFill();
        const points1 = determineTrianglePoints(NUM_POINTS, [
            [500, 200],
            [700, 250],
            [300, 750],
        ]);

        const points2 = determineTrianglePoints(NUM_POINTS, INNER_TRIANGLE);
    ...
    function determineTrianglePoints(numPoints: number, vertices: number[][]) {
        ...
        for (let i = 0; i < numPoints; i++) {
            const dist = i * spaceBetweenPoints - prevSideLengths;
            const pt = sideSegments[currSideSegmentIndex].pointAtLength(dist);
            if (pt == null){
