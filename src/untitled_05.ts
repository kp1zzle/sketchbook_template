...
const sketch = (s: p5SVG) => {
    ...
    s.draw = () =>  {
        ...
        s.stroke(255);
        s.noFill();
        const center = [s.windowWidth/2, s.windowHeight/2];
        const points1 = determinePointsOnCircle(NUM_POINTS, center, 80);

        const points2 = determinePointsOnCircle(NUM_POINTS, center, 300);

        const points3 = determinePointsOnCircle(NUM_POINTS * 2, center, 400);
        const points4 = determinePointsOnCircle(NUM_POINTS * 2, center, 450);
    ...
    function determinePointsOnCircle(numPoints: number, center: number[], radius: number) {
        const arc = Flatten.circle(Flatten.point(center[0], center[1]), radius).toArc();
        const spaceBetweenPoints = arc.length / numPoints;
        const outputPoints = [];
        for (let i = 0; i < numPoints; i++) {
            const dist = i * spaceBetweenPoints;
