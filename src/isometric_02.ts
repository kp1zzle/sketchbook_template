    ...
    s.draw = () =>  {
        ...
        for (let x = 0; x <= 500; x += 10) {
            ...
            for (let y = 0; y <= 500; y += 10) {
                ...
                let pt = convertToIsometric(s,{x: x, y: -heightFunc(x, y), z: y}, THETA)
                //vertex(pt.x, pt.y)
            }
            s.endShape();
        }
        ...
        var endPoint;
        for (let i = 0; i < 4; i++) {
            let minHeight = s.min(borderHeights)
            let x = borderPoints[i].x
            let y = borderPoints[i].y
            let upperPt = convertToIsometric(s,{x: x, y: -heightFunc(x, y), z: y }, THETA)
            let lowerPt = convertToIsometric(s,{x: x, y: minHeight - 50, z: y }, THETA)
